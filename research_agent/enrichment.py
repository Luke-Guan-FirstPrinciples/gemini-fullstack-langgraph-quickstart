...
# Patterns used to extract citation counts from open-web search snippets.
# Ordered from most specific to least specific.
_CITATION_EXTRACT_PATTERNS: list[re.Pattern[str]] = [
    re.compile(r"cited\s+by\s+(\d[\d,]*)", re.IGNORECASE),
    re.compile(r"citations?:\s*(\d[\d,]*)", re.IGNORECASE),
    re.compile(r"(\d[\d,]*)\s+citations?\b", re.IGNORECASE),
]


class WebSearchCitationsEnricher:
    """Extract a third citation-count signal from generic web search results.

    The enricher uses the configured :class:`SearchProvider` to look up each
    paper (by title) and then regex-scrapes ``Cited by N`` style fragments
    from the snippets. It's inherently best-effort: if the search provider
    doesn't surface a count, the paper's ``web_search`` enrichment stays in
    ``not_found`` state.
    """

    def __init__(self, cfg: Settings, search_provider: SearchProvider) -> None:
        self._cfg = cfg
    ...
    async def enrich_papers(self, papers: list[Paper]) -> list[Paper]:
        ...
        if not self._cfg.web_search_citations_enabled:
            logger.info("Web-search citations enrichment disabled by config")
            return [
                _apply_web_search_enrichment(paper, PaperWebSearchCitations(status="skipped"))
                for paper in papers
            ]

        max_papers = max(0, self._cfg.web_search_citations_max_papers)
        eligible_indexes = list(range(len(papers)))
        if max_papers and len(eligible_indexes) > max_papers:
            eligible_indexes = eligible_indexes[:max_papers]

        unique_lookups: dict[str, int] = {}
        for index in eligible_indexes:
            key = _lookup_key_for_paper(papers[index])
            if key and key not in unique_lookups:
                unique_lookups[key] = index

        lookup_to_enrichment: dict[str, PaperWebSearchCitations] = {}
        semaphore = asyncio.Semaphore(
            max(1, self._cfg.web_search_citations_parallelism)
        )

        async def _fetch_one(lookup_key: str, paper: Paper) -> None:
            async with semaphore:
                lookup_to_enrichment[lookup_key] = await self._fetch_for_paper(paper)

        await asyncio.gather(
            *[
                _fetch_one(lookup_key, papers[index])
                for lookup_key, index in unique_lookups.items()
            ]
        )
        ...
        for index, paper in enumerate(papers):
            if index not in eligible_indexes:
                enriched.append(
                    _apply_web_search_enrichment(
                        paper, PaperWebSearchCitations(status="skipped")
                    )
                )
                continue
            key = _lookup_key_for_paper(paper)
            enrichment = lookup_to_enrichment.get(key or "") or PaperWebSearchCitations()
            enriched.append(_apply_web_search_enrichment(paper, enrichment))

        matched = sum(
            1
            for paper in enriched
            if paper.web_search and paper.web_search.status == "matched"
        )
        logger.info(
            "Web-search citation enrichment matched %d/%d papers",
            matched,
            len(enriched),
        )
        return enriched

    async def _fetch_for_paper(self, paper: Paper) -> PaperWebSearchCitations:
        title = (paper.title or "").strip()
        if not title:
            return PaperWebSearchCitations(status="not_found")

        query = self._build_query(paper)
        try:
            results = await self._provider.search(
                query,
                num_results=max(1, self._cfg.web_search_citations_results_per_paper),
            )
        except Exception as exc:
            logger.exception("Web-search citation lookup failed for %s", title)
            return PaperWebSearchCitations(status="error", error=str(exc), query=query)

        best: tuple[int, SearchResult] | None = None
        for result in results or []:
            if not isinstance(result, SearchResult):
                continue
            count = _extract_citation_count(result.snippet) or _extract_citation_count(
                result.title
            )
            if count is None:
                continue
        ...
        if best is None:
            return PaperWebSearchCitations(status="not_found", query=query)

        count, result = best
        return PaperWebSearchCitations(
            status="matched",
            citation_count=count,
            source_url=result.url or None,
            source_display_name=result.source or None,
            snippet=result.snippet or None,
            query=query,
        )

    def _build_query(self, paper: Paper) -> str:
        template = (
            self._cfg.web_search_citations_query_template
            or '"{title}" "cited by"'
        )
        try:
            ...
            # Guard against misconfigured templates that reference unknown keys.
            return f'"{paper.title or ""}" "cited by"'


def _extract_citation_count(text: str | None) -> int | None:
    if not text:
        return None
    for pattern in _CITATION_EXTRACT_PATTERNS:
...
def _apply_web_search_enrichment(
    paper: Paper,
    enrichment: PaperWebSearchCitations,
) -> Paper:
    enriched = paper.model_copy(deep=True)
    enriched.web_search = enrichment
    return enriched
